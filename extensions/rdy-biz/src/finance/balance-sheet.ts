...
async function computeAccountBalances(
  ...
  let query = db.client
    .from("journal_lines")
    .select(
      "account_id, debit, credit, account:chart_of_accounts!account_id(code, name, type), journal_entry:journal_entries!journal_entry_id(date, status, book_id)",
    )
  ...
  if (bookId) {
    query = query.eq("journal_entry.book_id", bookId);
  }
  ...
  for (const line of data ?? []) {
    ...
    const acct = line.account as unknown as Record<string, unknown>;
    if (!grouped[acctId]) {
      grouped[acctId] = {
...
export function createBalanceSheetTool(config: BizConfig) {
  ...
  return {
    ...
    parameters: Type.Object({
      ...
      book: Type.Optional(
        Type.String({
          description: "Book code (e.g. 'statutory', 'internal'). Defaults to default book.",
        }),
      ),
    }),
    ...
    async execute(_id: string, params: Record<string, unknown>): Promise<ToolResult> {
      ...
      try {
        ...
        const currentBalances = await computeAccountBalances(db, asOfDate, bookId);

        let comparisonBalances: PeriodAccountBalance[] | null = null;
        if (comparisonDate) {
          comparisonBalances = await computeAccountBalances(db, comparisonDate, bookId);
        }
