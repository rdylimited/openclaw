...
export const BizConfigValues = z.object({
  ...
  chartOfAccountsPreset: z.enum(["hk-sme", "cn-sme", "us-gaap", "custom"]).default("hk-sme"),
  multiCurrency: z.boolean().default(true),
  taxJurisdiction: z.string().default("HK"),
  profitsTaxRate: z.number().min(0).max(1).default(0.165),
  cnEnterpriseType: z.enum(["standard", "small_micro", "high_tech"]).optional(),
  cnVatTaxpayerType: z.enum(["general", "small_scale"]).optional(),
  cnCity: z.string().optional(),
});
