...
function renderPayslip(
  ...
  // Dynamically iterate deduction items (jurisdiction-agnostic)
  const deductions = item.deductions ?? {};
  const deductionEntries = Object.entries(deductions);
  let totalDeductions = money("0");

  const deductionLines = deductionEntries.map(([label, amount]) => {
    const amt = money(amount);
    totalDeductions = totalDeductions.plus(amt);
    return `  ${label.padEnd(33)}${formatMoney(amt, currency).padStart(15)}`;
  });
