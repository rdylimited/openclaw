...
export default function (api: any) {
  ...
  api.registerTool({
    ...
    parameters: {
      ...
      properties: {
        ...
        category: { type: "string", description: "rent, staff, utilities, equipment, marketing, other" },
        name: { type: "string", description: "Cost description e.g. 'Monthly rent', 'Part-time staff'" },
        ...
        effective_to: { type: "string", description: "YYYY-MM-DD end date (optional, NULL = ongoing)" },
        notes: { type: "string" },
      },
      required: ["action"],
    },
    async execute(_id: string, p: any) {
      ...
      if (p.action === "create") {
        ...
        const { rows } = await pools.rdycore.query(
          ...
          [wsId, p.category, p.name, amountCents, p.frequency,
           p.effective_from ?? todayHK(), p.effective_to ?? null, p.notes ?? null]
        );
        return text({ message: "Cost added", cost: { ...rows[0], amount_hkd: rows[0].amount_cents / 100 } });
      }
      ...
      if (p.action === "update") {
        ...
        if (p.amount) { sets.push(`amount_cents = $${idx++}`); vals.push(Math.round(p.amount * 100)); }
        if (p.category) { sets.push(`category = $${idx++}`); vals.push(p.category); }
        if (p.frequency) { sets.push(`frequency = $${idx++}`); vals.push(p.frequency); }
        if (p.effective_to !== undefined) { sets.push(`effective_to = $${idx++}`); vals.push(p.effective_to); }
        if (p.notes !== undefined) { sets.push(`notes = $${idx++}`); vals.push(p.notes); }
        ...
        return text({ message: "Cost updated", cost: { ...rows[0], amount_hkd: rows[0].amount_cents / 100 } });
      }

      if (p.action === "delete") {
        if (!p.cost_id) return text({ error: "cost_id required for delete" });
        const { rowCount } = await pools.rdycore.query(
          "DELETE FROM business_costs WHERE id = $1", [p.cost_id]
        );
  ...
  api.registerTool({
    ...
    parameters: {
      ...
      properties: {
        ...
        target_value: { type: "number", description: "Target number (e.g. 70 for 70% utilization)" },
    ...
    async execute(_id: string, p: any) {
      ...
      if (p.action === "set") {
        ...
        if (existing.length > 0) {
          ...
          return text({ message: "Goal updated", metric: p.metric, target: p.target_value, period: p.period_type });
        }
        const { rows } = await pools.rdycore.query(
      ...
      if (p.action === "delete") {
        ...
        await pools.rdycore.query(
          "UPDATE profitability_goals SET is_active = false WHERE id = $1", [p.goal_id]
        );
  ...
  async function collectDailySnapshot(workspaceId: string, date: string): Promise<Record<string, any>> {
    ...
    const { rows: wsRows } = await pools.rdycore.query(
      "SELECT config FROM workspaces WHERE id = $1", [workspaceId]
    );
    ...
    const avgTicketCents = stats.booking_count > 0
      ? Math.round(stats.revenue_cents / stats.booking_count)
      : 0;
    ...
    // Margin
    const marginPct = stats.revenue_cents > 0
  ...
  api.registerTool({
    ...
    parameters: {
      ...
      properties: {
        workspace_id: { type: "string", description: "Auto-injected" },
        period_type: { type: "string", description: "daily, weekly, monthly, or annual (default: monthly)" },
        period_key: { type: "string", description: "Period key e.g. 2026-03-01, 2026-W09, 2026-03, 2026 (default: current)" },
      },
      required: [],
    },
    async execute(_id: string, p: any) {
      ...
      if (!snapshots || snapshots.length === 0) {
        return text({ error: `No snapshot data for ${periodType} ${periodKey}. Run take_snapshot first.` });
      }

      // Aggregate
      const totals = {
        revenue_cents: 0, cost_cents: 0, booking_count: 0, cancelled_count: 0,
        unique_customers: 0, booked_days: snapshots.length,
        total_utilization: 0,
      };
      ...
      const avgUtilization = totals.booked_days > 0
        ? Math.round((totals.total_utilization / totals.booked_days) * 100) / 100
        : 0;
      const avgTicket = totals.booking_count > 0
        ? Math.round(totals.revenue_cents / totals.booking_count)
        : 0;
      const marginPct = totals.revenue_cents > 0
        ? Math.round(((totals.revenue_cents - totals.cost_cents) / totals.revenue_cents) * 10000) / 100
        : 0;
      ...
      const goalScores = goals.map((g: any) => {
        let actual: number;
        switch (g.metric) {
          case "utilization_pct": actual = avgUtilization; break;
          case "margin_pct": actual = marginPct; break;
          case "revenue_monthly": actual = totals.revenue_cents / 100; break;
          case "bookings_daily": actual = totals.booked_days > 0 ? totals.booking_count / totals.booked_days : 0; break;
          case "avg_ticket": actual = avgTicket / 100; break;
          default: actual = 0;
        }
        const pct = g.target_value > 0 ? Math.round((actual / g.target_value) * 100) : 0;
        const status = pct >= 100 ? "on_target" : pct >= 80 ? "close" : "behind";
        return { metric: g.metric, target: g.target_value, actual: Math.round(actual * 100) / 100, pct, status };
      });
  ...
  api.registerTool({
    ...
    parameters: {
      ...
      properties: {
        ...
        category: { type: "string", description: "pricing, utilization, membership, cost_reduction, marketing, staffing, cross_sell" },
        expected_impact_pct: { type: "number", description: "Expected improvement percentage (for create)" },
        priority: { type: "string", description: "high, medium, low (default: medium)" },
        measured_impact_pct: { type: "number", description: "Actual measured impact (for measure)" },
        notes: { type: "string" },
      },
      required: ["action"],
    },
    async execute(_id: string, p: any) {
      ...
      if (p.action === "create") {
        ...
        const { rows } = await pools.rdycore.query(
          ...
          [wsId, p.recommendation, p.category, p.expected_impact_pct ?? null,
           p.priority ?? "medium", p.notes ?? null]
        );
  ...
  api.registerTool({
    ...
    parameters: {
      ...
      properties: {
        workspace_id: { type: "string", description: "Auto-injected" },
        report_type: { type: "string", description: "daily_brief, weekly_digest, monthly_review, annual_review" },
    ...
    async execute(_id: string, p: any) {
      ...
      if (p.report_type === "daily_brief") {
        ...
        if (existing.length > 0) {
          metrics = typeof existing[0].metrics === "string" ? JSON.parse(existing[0].metrics) : existing[0].metrics;
        } else {
        ...
        return text({
          ...
          goals: goals.map((g: any) => {
            let actual: number;
            switch (g.metric) {
              case "utilization_pct": actual = metrics.utilization_pct; break;
              case "bookings_daily": actual = metrics.booking_count; break;
              case "avg_ticket": actual = metrics.avg_ticket_cents / 100; break;
              default: actual = 0;
            }
            const status = actual >= g.target_value ? "hit" : actual >= g.target_value * 0.8 ? "close" : "missed";
            return { metric: g.metric, target: g.target_value, actual: Math.round(actual * 100) / 100, status };
          }),
      ...
      if (p.report_type === "weekly_digest") {
        ...
        const sumMetrics = (snaps: any[]) => {
          const totals = { revenue: 0, cost: 0, bookings: 0, utilization: 0, customers: 0, days: snaps.length };
        ...
        return text({
          ...
          this_week: {
            ...
            avg_utilization_pct: thisWeek.days > 0 ? Math.round((thisWeek.utilization / thisWeek.days) * 100) / 100 : 0,
      ...
      if (p.report_type === "monthly_review") {
        ...
        const aggregateSnaps = (snaps: any[]) => {
          const t = { revenue: 0, cost: 0, bookings: 0, cancelled: 0, utilization: 0, customers: 0, days: snaps.length };
          for (const s of snaps) {
            const m = typeof s.metrics === "string" ? JSON.parse(s.metrics) : s.metrics;
        ...
        const marginPctMonth = thisMonth.revenue > 0
          ? Math.round(((thisMonth.revenue - thisMonth.cost) / thisMonth.revenue) * 10000) / 100
          : 0;
        const avgTicketMonth = thisMonth.bookings > 0
          ? Math.round(thisMonth.revenue / thisMonth.bookings)
          : 0;

        const goalScores = goals.map((g: any) => {
          let actual: number;
          switch (g.metric) {
            case "utilization_pct": actual = avgUtil; break;
            case "margin_pct": actual = marginPctMonth; break;
            case "revenue_monthly": actual = thisMonth.revenue / 100; break;
            case "bookings_daily": actual = thisMonth.days > 0 ? thisMonth.bookings / thisMonth.days : 0; break;
            case "avg_ticket": actual = avgTicketMonth / 100; break;
            default: actual = 0;
          }
          const pct = g.target_value > 0 ? Math.round((actual / g.target_value) * 100) : 0;
          const status = pct >= 100 ? "on_target" : pct >= 80 ? "close" : "behind";
        ...
        const breakEvenStatus = dailyRevAvg > dailyCostAvg
          ? "profitable"
          : dailyRevAvg > 0
        ...
        return text({
          ...
          demand_forecast: {
            avg_daily_bookings: Math.round(Number(weekdayAvg[0]?.avg_daily_bookings ?? 0) * 100) / 100,
            projected_next_month_bookings: Math.round(Number(weekdayAvg[0]?.avg_daily_bookings ?? 0) * 30),
          },
          delivery_instruction:
      ...
      if (p.report_type === "annual_review") {
        ...
        for (const snap of yearSnaps) {
          ...
          if (!quarters[q]) quarters[q] = { revenue: 0, cost: 0, bookings: 0, cancelled: 0, utilization: 0, customers: 0, days: 0 };
          const m = typeof snap.metrics === "string" ? JSON.parse(snap.metrics) : snap.metrics;
  ...
  api.registerTool({
    ...
    async execute(_id: string, p: any) {
      ...
      const heatmap: Record<string, { hour: number; avg_bookings: number; capacity: number; fill_pct: number }[]> = {};

      for (const row of patterns) {
        const dayKey = dayNames[row.dow];
        if (!heatmap[dayKey]) heatmap[dayKey] = [];
        const avgPerDay = row.sample_days > 0 ? Math.round((row.avg_bookings / row.sample_days) * 100) / 100 : 0;
        const fillPct = resourceCount > 0 ? Math.round((avgPerDay / resourceCount) * 10000) / 100 : 0;
        heatmap[dayKey].push({
          hour: row.hour,
      ...
      const forecastDateStr = forecastDate.toLocaleDateString("sv-SE", { timeZone: "Asia/Hong_Kong" });
      ...
      const predictedUtilization = historicalForDay.length > 0
        ? Math.round(historicalForDay.reduce((s, h) => s + h.fill_pct, 0) / historicalForDay.length * 100) / 100
        : 0;
  ...
  api.registerTool({
    ...
    async execute(_id: string, p: any) {
      ...
      if (dbName === "racing") {
        ...
        const dateFilter = periodType === "daily"
          ? "booking_date = $2"
          : periodType === "weekly"
        ...
        for (const rig of rigs) {
          ...
          const { rows: rigStats } = await pool.query(
            ...
            periodType === "weekly"
              ? [wsId, null, rig.id]
              : [wsId, periodKey, rig.id]
          );

          const s = rigStats[0];
          const wsRows = await pools.rdycore.query("SELECT config FROM workspaces WHERE id = $1", [wsId]);
          ...
          const utilPct = totalMinutes > 0 ? Math.round((s.booked_minutes / totalMinutes) * 10000) / 100 : 0;
          ...
          await pools.rdycore.query(
            ...
            [wsId, rig.slug, periodType, periodKey, s.revenue_cents, maintCents, s.booking_count, utilPct]
          );
        }
      ...
      const bs = bayStats[0];
      const wsConfigRows = await pools.rdycore.query("SELECT config FROM workspaces WHERE id = $1", [wsId]);
      const opMin = getOperatingMinutes(wsConfigRows.rows[0]?.config ?? {});
      const dayCount = periodType === "daily" ? 1 : 30;
      const utilPctGolf = (opMin * dayCount) > 0 ? Math.round((bs.booked_minutes / (opMin * dayCount)) * 10000) / 100 : 0;
    ...
    async execute(_id: string, p: any) {
      ...
      for (const cs of coachStats) {
        ...
        if (cs.unique_customers > 0) {
          ...
          repeatRate = cs.unique_customers > 0
        ...
        coachResults.push({
          ...
          revenue_per_hour_hkd: hoursWorked > 0 ? Math.round((cs.revenue_cents / hoursWorked) / 100) : 0,
  ...
  async function generatePricingSuggestions(workspaceId: string): Promise<void> {
    ...
    const tomorrowFillPct = (resourceCount * 13) > 0
      ? (tomorrowBookings[0].cnt / (resourceCount * 13)) * 100
      : 0;
    ...
    if (weeklyGaps.length >= 5) {
      const dayNames = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
      const gapSummary = weeklyGaps.slice(0, 5)
  ...
  api.registerTool({
    ...
    parameters: {
      ...
      properties: {
        workspace_id: { type: "string", description: "Auto-injected" },
        risk_level: { type: "string", description: "Filter by risk: low, medium, high, churned (default: all at-risk)" },
    ...
    async execute(_id: string, p: any) {
      ...
      for (const member of members) {
        ...
        const risk = daysSince < 14 ? "low"
          : daysSince < 30 ? "medium"
          : daysSince < 60 ? "high"
          : "churned";

        const outreach = risk === "medium"
  ...
  api.registerTool({
    ...
    async execute(_id: string, p: any) {
      ...
      const totalCustomers = (walkIns[0]?.cnt ?? 0) + tierDist.reduce((s: number, t: any) => s + t.cnt, 0);
      const memberCount = tierDist.reduce((s: number, t: any) => s + t.cnt, 0);
      const conversionRate = totalCustomers > 0
        ? Math.round((memberCount / totalCustomers) * 10000) / 100
        : 0;
  ...
  api.registerTool({
    ...
    async execute(_id: string, p: any) {
      ...
      const revenueExpr = dbName === "racing" ? "b.final_price_cents" : "(b.final_price * 100)::int";
      const dateFilter = periodType === "daily"
        ? "b.booking_date = $2::date"
        : periodType === "weekly"
          ? "b.booking_date >= CURRENT_DATE - INTERVAL '7 days'"
  ...
  api.registerTool({
    ...
    parameters: {
      ...
      properties: {
        ...
        metric: { type: "string", description: "Metric name (e.g. price_per_hour, utilization_rate)" },
        value: { type: "number", description: "Benchmark value" },
        source: { type: "string", description: "manual, web_scrape, industry_report (default: manual)" },
      },
      required: ["action"],
    },
    async execute(_id: string, p: any) {
      ...
      if (p.action === "delete") {
        ...
        const { rowCount } = await pools.rdycore.query(
          "DELETE FROM competitor_benchmarks WHERE id = $1", [p.benchmark_id]
        );
      ...
      if (p.action === "compare") {
        ...
        if (ownSnaps.length > 0) {
          let totalRev = 0, totalBookings = 0, totalUtil = 0;
          ...
          ownMetrics["avg_ticket"] = totalBookings > 0 ? Math.round(totalRev / totalBookings) / 100 : 0;
        ...
        const comparisons = benchmarks.map((b: any) => {
          ...
          const diff = ownVal !== undefined && b.value > 0
          ...
          return {
            ...
            diff_pct: diff,
            status: diff === null ? "no data" : diff > 5 ? "ahead" : diff < -5 ? "behind" : "similar",
          };
        });
  ...
  api.registerTool({
    ...
    async execute(_id: string, p: any) {
      ...
      const breakEvenDays = avgDailyRevenue > 0
        ? Math.ceil(monthlyCostCents / avgDailyRevenue)
        : null;
      ...
      return text({
        ...
        break_even_status: breakEvenDays === null
          ? "no revenue data"
          : breakEvenDays <= 30
  ...
  api.registerTool({
    ...
    async execute(_id: string, p: any) {
      ...
      const avgRevenue = Math.round(monthlyHistory.reduce((s: number, m: any) => s + (m.revenue_cents ?? 0), 0) / monthlyHistory.length);
      const avgCost = Math.round(monthlyHistory.reduce((s: number, m: any) => s + (m.cost_cents ?? 0), 0) / monthlyHistory.length);
      const avgTopups = Math.round(monthlyHistory.reduce((s: number, m: any) => s + (m.topups_cents ?? 0), 0) / monthlyHistory.length);
      const avgRefunds = Math.round(monthlyHistory.reduce((s: number, m: any) => s + (m.refunds_cents ?? 0), 0) / monthlyHistory.length);

      // Growth trend (simple linear)
      const recentRevenue = monthlyHistory.length >= 2 ? (monthlyHistory[0]?.revenue_cents ?? 0) : avgRevenue;
      const olderRevenue = monthlyHistory.length >= 2 ? (monthlyHistory[monthlyHistory.length - 1]?.revenue_cents ?? 0) : avgRevenue;
      ...
      for (let i = 1; i <= periodsAhead; i++) {
        ...
        const monthKey = projDate.toLocaleDateString("sv-SE", { timeZone: "Asia/Hong_Kong" }).slice(0, 7);
  ...
  api.registerTool({
    ...
    async execute(_id: string, p: any) {
      ...
      const XERO_REDIRECT_URI = process.env.XERO_REDIRECT_URI ?? "https://openclaw.rdylimited.com/webhooks/xero-callback";
      const XERO_SCOPES = "openid profile email accounting.transactions.read accounting.contacts.read offline_access";
      ...
      if (p.action === "status") {
        ...
        if (!xero || !xero.access_token) {
          return text({ connected: false, message: "Xero not connected. Use action=connect to start OAuth." });
        }
        return text({
          connected: true,
      ...
      if (p.action === "connect") {
        if (!XERO_CLIENT_ID) {
          return text({ error: "XERO_CLIENT_ID not configured. Set env vars XERO_CLIENT_ID and XERO_CLIENT_SECRET." });
        }
        ...
        return text({
          ...
          auth_url: authUrl,
          instructions: "After authorizing, Xero will redirect with a code. Use action=callback with that code.",
        });
      }
      ...
      if (p.action === "callback") {
        ...
        try {
          ...
          if (!tokenRes.ok) {
            return text({ error: `Token exchange failed: ${tokenRes.status} ${await tokenRes.text()}` });
          }
          ...
          return text({ message: `Connected to Xero: ${tenant?.tenantName}. Use action=sync to pull expenses.` });
        } catch (err: any) {
      ...
      if (p.action === "sync") {
        ...
        const categoryMap: Record<string, string> = {
          ...
          "salary": "staff", "wage": "staff", "payroll": "staff",
          "electric": "utilities", "water": "utilities", "internet": "utilities", "phone": "utilities",
          "repair": "equipment", "maintenance": "equipment", "hardware": "equipment",
          "advertising": "marketing", "google ads": "marketing", "facebook": "marketing", "promotion": "marketing",
        };
        ...
        for (const inv of invoices) {
          for (const line of inv.LineItems ?? []) {
            ...
            for (const [keyword, cat] of Object.entries(categoryMap)) {
              if (desc.includes(keyword)) { category = cat; break; }
            }
            ...
            const { rowCount } = await pools.rdycore.query(
              ...
              [wsId, category, line.Description ?? `Xero: ${inv.InvoiceNumber}`, amountCents, effectiveDate, `Xero Invoice: ${inv.InvoiceNumber}`]
            );
      ...
      if (p.action === "disconnect") {
        await pools.rdycore.query(
          `UPDATE workspaces SET config = config - 'xero' WHERE id = $1`,
          [wsId]
        );
  ...
  api.registerTool({
    ...
    parameters: {
      ...
      properties: {
        ...
        account_name: { type: "string", description: "Bank account name (e.g. 'HSBC Operating', 'Hang Seng Savings')" },
        balance: { type: "number", description: "Current balance in HKD (for record action)" },
        csv_data: { type: "string", description: "CSV content: date,description,amount,balance per line (for import_csv)" },
    ...
    async execute(_id: string, p: any) {
      ...
      if (p.action === "record") {
        ...
        await pools.rdycore.query(
          ...
          [wsId, today, JSON.stringify({ [p.account_name]: balanceCents, recorded_at: new Date().toISOString() })]
        );
      ...
      if (p.action === "current") {
        ...
        const m = typeof rows[0].metrics === "string" ? JSON.parse(rows[0].metrics) : rows[0].metrics;
      ...
      if (p.action === "import_csv") {
        if (!p.csv_data) return text({ error: "csv_data required (date,description,amount,balance per line)" });
  ...
  api.registerTool({
    ...
    parameters: {
      ...
      properties: {
        ...
        csv_data: { type: "string", description: "CSV: competitor_name,metric,value[,source] per line" },
        benchmarks: {
          type: "array",
    ...
    async execute(_id: string, p: any) {
      ...
      if (p.action === "import_csv") {
        ...
        return text({ message: `Imported ${imported} benchmarks from CSV`, lines_processed: lines.length });
      }
      ...
      if (p.action === "scrape_hint") {
        ...
        const hints = vertical === "golf" ? {
          vertical: "sim_golf",
          suggested_competitors: [
            "Golf Zone HK", "X-Golf HK", "Par-Tee Time", "industry_average"
          ],
          metrics_to_track: [
            ...
            { metric: "utilization_rate", description: "Estimated utilization % (from reviews/observations)" },
          ...
          vertical: "sim_racing",
          suggested_competitors: [
            "Apex Racing HK", "SimGrid HK", "Race Room HK", "industry_average"
          ],
          metrics_to_track: [
            ...
            { metric: "group_discount_pct", description: "Group booking discount percentage" },
        ...
        return text({
          ...
          ...hints,
          import_instruction: "Once you have data, use action=import_csv with format: competitor_name,metric,value,source per line. Or provide it conversationally and I'll use action=import_structured.",
        });
      }
