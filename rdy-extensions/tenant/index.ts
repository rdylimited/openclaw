...
const PG_URL =
  process.env.RDY_PG_URL ?? "postgresql://postgres@localhost:5433/rdycore";
...
async function resolveTenantByPhone(phone: string): Promise<Tenant | null> {
  const { rows } = await queryDb(
    "SELECT id, phone, name, plan FROM tenants WHERE phone = $1",
    [phone],
  );
  return rows[0] ?? null;
}
...
function extractPhoneFromSessionKey(
...
async function resolveAndCacheByIdentity(
...
function buildMultiWorkspacePrompt(
  tenant: Tenant,
  workspaces: Workspace[],
): string {
...
function inferVerticalFromParams(
  toolName: string,
...
function handleToolInjection(
  ...
  if (suppliedId && !UUID_RE.test(suppliedId)) {
    ...
    const match = tenantCtx.workspaces.find(
      (w) =>
        w.slug === input ||
        w.name.toLowerCase() === input ||
        verticalLabel(w.vertical) === input,
    );
    if (match) {
      log.info(
  ...
  log.info(
    `[rdy-tenant] auto-injecting workspace_id=${workspaceId} into ${event.toolName}`,
  );
...
export default function (api: any) {
  ...
  api.on("message_received", async (event: any, ctx: any) => {
    ...
    if (tenantCtx) {
      ...
      log.info(
        `[rdy-tenant] no tenant found for ${identity.channel}:${identity.channelUid}`,
      );
    }
  });
  ...
  api.on(
    ...
    async (event: any, ctx: any) => {
      ...
      if (overLimit) {
        log.info(
          `[rdy-tenant] injecting rate-limit prompt for session ${ctx.sessionKey}`,
        );
      ...
      if (!tenantId) {
        ...
        const fallbackIdentity =
          identity ?? (() => {
            ...
            return phone
              ? { channel: "whatsapp" as const, channelUid: phone }
              : null;
          })();
    ...
    async execute(
      ...
      try {
        ...
        if (!identity) {
          return text({
            error:
              "Cannot detect your current channel. Please send a message first.",
          });
        }
        ...
        if (identity.channel === "whatsapp") {
          return text({
            error:
              "You're already on WhatsApp — your account should be linked automatically.",
          });
        }
    ...
    parameters: {
      ...
      properties: {
        phone: {
          type: "string",
          description:
            "Tenant phone number in E.164 format (optional, auto-detected from session)",
        },
      },
    },
    async execute(
      ...
      try {
        ...
        const tenantCtx = await resolveAndCache(phone);
        if (!tenantCtx)
          return text({ error: `No tenant found for phone ${phone}` });
  ...
  api.registerCommand({
    ...
    async handler(ctx: any) {
      try {
        ...
        log.info(`[rdy-tenant] /golf command: from=${phone}, sessionKey=${ctx.sessionKey}, keys=${Object.keys(ctx).join(",")}`);
        if (phone) {
          const tenantCtx = await resolveAndCache(phone);
          if (tenantCtx) {
            const golfWs = tenantCtx.workspaces.find(
              (w) => w.vertical === "sim_golf",
            );
            if (golfWs) {
    ...
    async handler(ctx: any) {
      try {
        ...
        if (phone) {
          const tenantCtx = await resolveAndCache(phone);
          if (tenantCtx) {
            const racingWs = tenantCtx.workspaces.find(
              (w) => w.vertical === "sim_racing",
            );
            if (racingWs) {
