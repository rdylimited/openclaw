...
import { processKfNotification } from "./kf-handler.js";
import {
...
async function handleMessageCallback(req, res, crypto, agentConfig, config) {
  try {
    ...
    // ── Customer Service (微信客服) event routing ──
    // If the decrypted XML contains Event=kf_msg_or_event, route to KF handler
    const eventType = (msg.Event ?? "").toLowerCase();
    if (msgType === "event" && eventType === "kf_msg_or_event") {
      logger.info("[agent-inbound] routing to KF handler (kf_msg_or_event)");
      res.writeHead(200, { "Content-Type": "text/plain; charset=utf-8" });
      res.end("success");

      // Extract Token from XML for sync_msg
      const syncToken = msg.Token ?? "";
      processKfNotification({ agentConfig, config, syncToken }).catch((err) => {
        logger.error("[agent-inbound] KF async processing failed", { error: err.message });
      });
      return true;
    }
  ...
  if (["image", "voice", "video", "file"].includes(msgType)) {
    ...
    if (mediaId) {
      try {
        ...
        if (msgType === "voice" && saved.path) {
          const transcript = await transcribeAudio(saved.path);
          if (transcript) {
            finalContent = transcript;
            logger.info("[agent-inbound] voice transcribed via ASR", { preview: transcript.substring(0, 80) });
          }
        }

        // For image-only messages, set a placeholder body.
        if (msgType === "image" && (!content.trim() || content.startsWith("[图片]"))) {
