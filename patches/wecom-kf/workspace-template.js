 ...
 * KF agents use channels.wecom.kfWorkspaceTemplate (lean prompt),
 * falling back to channels.wecom.workspaceTemplate.
 */
export function getWorkspaceTemplateDir(config, source) {
  if (source === "kf") {
    const kfDir = config?.channels?.wecom?.kfWorkspaceTemplate?.trim();
    if (kfDir) return kfDir;
  }
 ...
 */
export function seedAgentWorkspace(agentId, config, source) {
  const templateDir = getWorkspaceTemplateDir(config, source);
  if (!templateDir) {
    return;
  }
...
export async function ensureDynamicAgentListed(agentId, { source } = {}) {
  const normalizedId = String(agentId || "")
    .trim()
    .toLowerCase();
  if (!normalizedId) {
  ...
  const queue = getEnsureDynamicAgentWriteQueue()
    .then(async () => {
      ...
      seedAgentWorkspace(normalizedId, latestConfig, source);
